...
>(({ className, ...props }, ref) => (
  <AccordionPrimitive.Item
    ref={ref}
    className={cn(
      "border-b border-white/10 mb-3 rounded-2xl bg-card/40 border px-6 transition-all duration-200 hover:border-primary/30",
      className
    )}
    {...props}
  />
...
>(({ className, children, ...props }, ref) => (
  <AccordionPrimitive.Content
    ...
    {...props}
  >
    <div className={cn("pb-5 pt-0 text-muted-foreground/90 leading-relaxed", className)}>
      {children}
    </div>
  </AccordionPrimitive.Content>
));
